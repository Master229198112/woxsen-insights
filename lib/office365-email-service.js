...
class Office365EmailService {
  ...
  async sendBulkEmails({ emails, subject, html, text = null }) {
    const batchSize = 30; // Office365 rate limit: ~30 emails/minute
    ...
    console.log(`📧 Starting bulk email send to ${emails.length} recipients via Office365`);
  ...
  async sendNewsletterResume({ subscribers, subject, content, newsletterId }) {
    console.log(`🔄 Resuming newsletter "${subject}" to ${subscribers.length} recipients via Office365`);
    ...
    const batchSize = 20; // Conservative batch size for resume operations
    const delayBetweenBatches = 180000; // 3 minutes between batches
    ...
    for (let i = 0; i < subscribers.length; i += batchSize) {
      ...
      console.log(`📦 Processing resume batch ${batchNumber} (${batch.length} subscribers)`);
      ...
      for (const subscriber of batch) {
        try {
          ...
          // Delay between individual newsletter emails
          await this.delay(4000); // 4 seconds between emails for resume operations
        } catch (error) {
