...
class EmailService {
  ...
  /**
   * Resume sending newsletter to failed/unsent recipients
   */
  async sendNewsletterResume({ subscribers, subject, content, newsletterId }) {
    console.log(`🔄 Resuming newsletter "${subject}" to ${subscribers.length} recipients`);
    
    if (this.useOffice365) {
      return await this.emailService.sendNewsletterResume({ subscribers, subject, content, newsletterId });
    } else {
      return await this.sendNewsletterResumeWithSendGrid({ subscribers, subject, content, newsletterId });
    }
  }

  /**
   * Resume sending newsletter using SendGrid (legacy method)
   */
  async sendNewsletterResumeWithSendGrid({ subscribers, subject, content, newsletterId }) {
    const results = {
      successful: 0,
      failed: 0,
      errors: []
    ...
    const batchSize = 50; // Smaller batch size for resume operations
    ...
    for (let i = 0; i < subscribers.length; i += batchSize) {
      ...
      try {
        ...
        const deliveryRecords = batch.map(subscriber => ({
          newsletterId,
        ...
        await NewsletterDelivery.insertMany(deliveryRecords);
        
        results.successful += batch.length;
        console.log(`✅ Resume batch ${Math.floor(i/batchSize) + 1} sent successfully (${batch.length} emails)`);
        
        if (i + batchSize < subscribers.length) {
          await this.delay(1000); // Rate limiting delay
        }
      } catch (error) {
        console.error(`❌ Resume batch ${Math.floor(i/batchSize) + 1} failed:`, error);
        
        // Record failed deliveries
        try {
          const failedRecords = batch.map(subscriber => ({
            newsletterId,
            email: subscriber.email,
            status: 'failed',
            attempts: 1,
          ...
          await NewsletterDelivery.insertMany(failedRecords);
        } catch (recordError) {
          console.error('Failed to record delivery failures:', recordError);
        }
